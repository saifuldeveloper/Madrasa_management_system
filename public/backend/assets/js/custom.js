...
function feeChargeShow() {
    var url = $('#url').val();
    var classs_id = $("#getSections").val();
    var frees_group_id = $("#fees_group").val();

    var formData = {
        class_id: classs_id,
        fees_id  :frees_group_id
    }
...
// Fees class assing end
// Fees master clsss type
...
function calculationData(){
    ...
    $('table').on('change', '.pay_amount, .donation_amount', function() {
        const index = $(this).data('index');
        const totalAmount = parseFloat($(`.total_amount[data-index="${index}"]`).text()) || 0;
        const payAmount = parseFloat($(`.pay_amount[data-index="${index}"]`).val()) || 0;
        const donationAmount = parseFloat($(`.donation_amount[data-index="${index}"]`).val()) || 0;
        const total = totalAmount - payAmount - donationAmount;
        console.log(total);

        $(`.payable[data-index="${index}"]`).text(total.toFixed(2));
    });
}
